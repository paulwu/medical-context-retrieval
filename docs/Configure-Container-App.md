...
2. Verify **System assigned** tab shows **Status: On**
3. Copy the **Object (principal) ID** — you'll need this for RBAC assignments
...
In `rag/embeddings.py` and other modules, update the OpenAI client initialization:
...
When using managed identity, you can omit `AZURE_OPENAI_API_KEY` entirely. The SDK will automatically use the Container App's system-assigned identity.
...
3. Ensure the `.env` file variables are configured as Container App secrets
