#!/usr/bin/env python3
"""
Setup Azure AI Search Indexer for Cosmos DB
This automatically indexes chunks from Cosmos DB and generates embeddings

Benefits over manual approach:
- Automatic embedding generation via Azure OpenAI skillset
- Keeps search index in sync with Cosmos DB
...
try:
    ...
    print("Run: pip install azure-search-documents azure-cosmos")
    sys.exit(1)
...
def create_data_source():
    ...
    data_source = SearchIndexerDataSourceConnection(
        ...
        container=SearchIndexerDataContainer(
            name=config.COSMOS_CONTAINER_CHUNKS,
    ...
    print(f"  Connected to: {config.COSMOS_DB_NAME}/{config.COSMOS_CONTAINER_CHUNKS}")
...
def create_skillset():
    ...
    embedding_skill = AzureOpenAIEmbeddingSkill(
        ...
        deployment_id=config.AOAI_EMBED_MODEL,
    ...
    print(f"  Using model: {config.AOAI_EMBED_MODEL}")
...
def create_indexer():
    ...
    # Field mappings from Cosmos DB to Index
    field_mappings = [
        FieldMapping(source_field_name="chunk_id", target_field_name="id"),
        # All other fields auto-map by name
    ]
    ...
    indexer = SearchIndexer(
        ...
        target_index_name=config.AZURE_SEARCH_INDEX_NAME,
...
def main():
    ...
    print(f"\nConfiguration:")
    print(f"  Cosmos DB: {config.COSMOS_ENDPOINT}")
    print(f"  Container: {config.COSMOS_CONTAINER_CHUNKS}")
    ...
    try:
        ...
        print("  • Or run: python3 check_indexer_status.py")
        print("=" * 70)
